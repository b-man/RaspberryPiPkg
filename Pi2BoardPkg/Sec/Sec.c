  ...
  )
{
  ...
  if (ArmPlatformIsPrimaryCore (MpId) && (SecBootMode == ARM_SEC_COLD_BOOT)) {
    ...
    ArmGicEnableInterruptInterface ((UINT32)FixedPcdGet64(PcdGicInterruptInterfaceBase));
  } else {
  ...
  if (FixedPcdGetBool (PcdTrustzoneSupport)) {
    ...
    ASSERT (((FixedPcdGet64(PcdCPUCoresSecMonStackBase) == 0) && (PcdGet32(PcdCPUCoreSecMonStackSize) == 0)) ||
            ((FixedPcdGet64(PcdCPUCoresSecMonStackBase) != 0) && (PcdGet32(PcdCPUCoreSecMonStackSize) != 0)));

    // Enter Monitor Mode
    enter_monitor_mode (
      (UINTN)TrustedWorldInitialization, MpId, SecBootMode,
      (VOID*) (((UINT32)FixedPcdGet64(PcdCPUCoresSecMonStackBase)) +
  ...
  )
{
  ...
  ArmGicSetupNonSecure (MpId, ((UINT32)FixedPcdGet64(PcdGicDistributorBase)), ((UINT32)FixedPcdGet64(PcdGicInterruptInterfaceBase)));
